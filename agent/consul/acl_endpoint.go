...
func (a *ACL) TokenSet(args *structs.ACLTokenSetRequest, reply *structs.ACLToken) error {
	...
	return a.tokenSetInternal(args, reply, false || a.disableLoginOnlyRestrictionOnTokenSet)
}
...
func (a *ACL) tokenSetInternal(args *structs.ACLTokenSetRequest, reply *structs.ACLToken, fromLogin bool) error {
	...
	if token.AccessorID == "" {
		...
		if fromLogin {
			...
			if !isValidIDPName(token.IDPName) {
				return fmt.Errorf("IDPName field is invalid. Only alphanumeric characters, '-' and '_' are allowed")
			}
		} else {
			if token.IDPName != "" {
